...
class SimulationEngine:
    ...
    def __init__(self, floorplan: dict, config: dict):
        ...
        if room_volume and room_volume > 0:
            ...
            self.thermal_capacity = 50 * AIR_DENSITY * \
                AIR_SPECIFIC_HEAT * THERMAL_MASS_MULTIPLIER

        # Allow config override
        if floorplan.get("thermal_mass"):
            # kJ/K to J/K
            self.thermal_capacity = floorplan["thermal_mass"] * 1000
    ...
    def run(self) -> dict:
        ...
        while current_second < total_seconds:
            ...
            loss_breakdown = self.heat_loss_calc.calculate_total_loss(
                indoor_temp)
            total_loss = loss_breakdown["total"]

            # Get heating power from controller
            time_minutes = current_second // 60
            heating_power, heating_on = self.heating_ctrl.get_heating_power(
                indoor_temp, time_minutes)
            ...
            for key in heat_loss_totals:
                heat_loss_totals[key] += loss_breakdown.get(
                    key, 0) * INTERNAL_TIMESTEP_SECONDS / 3600

            # Track energy usage
            if heating_on:
                total_energy_wh += heating_power * \
            ...
            if current_second - last_recording_second >= recording_interval_seconds:
                ...
                heating_ratio = heating_on_count / \
                    samples_in_interval if samples_in_interval > 0 else 0
            ...
            temp_change = (net_power * INTERNAL_TIMESTEP_SECONDS) / \
        ...
        if time_series:
            avg_temp = sum(p["indoor_temp"]
                           for p in time_series) / len(time_series)
