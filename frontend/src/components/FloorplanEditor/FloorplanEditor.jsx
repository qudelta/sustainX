...
export default function FloorplanEditor({ floorplan, onChange }) {
    ...
    const isRoomClosed = () => {
        ...
        const endpoints = [];
        walls.forEach((wall) => {
            endpoints.push({ x: wall.x1, y: wall.y1, wallId: wall.id, type: 'start' });
    ...
    const calculateFloorArea = () => {
        ...
        while (usedWalls.size < walls.length) {
            ...
            const nextWall = walls.find(w => {
                ...
                return (
                    (Math.abs(w.x1 - nextPoint.x) < tolerance && Math.abs(w.y1 - nextPoint.y) < tolerance) ||
                    (Math.abs(w.x2 - nextPoint.x) < tolerance && Math.abs(w.y2 - nextPoint.y) < tolerance)
                );
            });
            ...
            if (Math.abs(nextWall.x1 - nextPoint.x) < tolerance && Math.abs(nextWall.y1 - nextPoint.y) < tolerance) {
    ...
    const syncFloorCeilingArea = useCallback(() => {
        ...
        if (area > 0) {
            const areaM2 = parseFloat((area * FEET_TO_METERS * FEET_TO_METERS).toFixed(2));
            onChange({
                ...floorplan,
                ...
                <Paper withBorder p="md">
                    <Group justify="space-between" mb="md">
                        <Group gap="sm">
                            ...
                            {roomClosed ? (
                                <Badge color="green" variant="light">Room Closed ✓</Badge>
                            ) : (
                                <Badge color="orange" variant="light">Room Open</Badge>
                            )}
                        </Group>
                    ...
                    >
                        {/* Grid */}
                        {Array.from({ length: Math.floor(width / SCALE) + 1 }).map((_, i) => (
                            <line
                                key={`v${i}`}
                                x1={i * SCALE}
                                y1={0}
                        ...
                        {Array.from({ length: Math.floor(height / SCALE) + 1 }).map((_, i) => (
                            <line
                                key={`h${i}`}
                                x1={0}
                                y1={i * SCALE}
                        ...
                        {floorplan.walls?.map((wall) => {
                            ...
                            return (
                                <g key={wall.id}>
                                    ...
                                    {/* Windows */}
                                    {floorplan.windows?.filter(w => w.wall_id === wall.id).map((win) => {
                                        const dx = wall.x2 - wall.x1;
                                        const dy = wall.y2 - wall.y1;
                                    ...
                                    {/* Doors */}
                                    {floorplan.doors?.filter(d => d.wall_id === wall.id).map((door) => {
                                        const dx = wall.x2 - wall.x1;
                                        const dy = wall.y2 - wall.y1;
                    ...
                    <Text size="xs" c="dimmed" mt="xs">
                        1 grid = 1 foot (5 grid lines = 5 ft). Drag endpoints to position walls. Endpoints snap to nearby walls.
                    </Text>

                    {!roomClosed && floorplan.walls?.length > 0 && (
                        <Alert color="yellow" mt="sm" variant="light">
                            Connect all wall endpoints to close the room for accurate simulation.
                        </Alert>
                    )}
                </Paper>
            </Grid.Col>
                        ...
                        <Accordion.Item value="walls">
                            ...
                            <Accordion.Panel>
                                {selectedWall ? (
                                    <Stack gap="xs">
                                        {(() => {
                                            const wall = floorplan.walls?.find(w => w.id === selectedWall);
                                            if (!wall) return <Text c="dimmed" size="sm">Wall not found</Text>;

                                            const length = getWallLength(wall);
                                            const wallWindows = floorplan.windows?.filter(w => w.wall_id === wall.id) || [];
                                            const wallDoors = floorplan.doors?.filter(d => d.wall_id === wall.id) || [];
                                            ...
                                            return (
                                                <>
                                                    ...
                                                    />
                                                    <Group grow>
                                                        <Button size="xs" variant="light" onClick={() => addWindow(wall.id)}>
                                                            + Window
                                                        </Button>
                                                        <Button size="xs" variant="light" onClick={() => addDoor(wall.id)}>
                                                            + Door
                                                        </Button>
                                                    </Group>
                                                    ...
                                                    {wallWindows.length > 0 && (
                                                        <>
                                                            ...
                                                            {wallWindows.map((win, idx) => (
                                                                <Card key={win.id} withBorder padding="xs" bg="blue.0">
                                                                    <Text size="xs" fw={500} mb="xs">Window {idx + 1}</Text>
                                                                    <Group grow mb="xs">
                                                                        <NumberInput
                                                                            label="Width (ft)"
                                                                            value={win.width}
                                                                            onChange={(v) => updateWindow(win.id, { width: v })}
                                                                            step={0.5}
                                                                            min={0.5}
                                                                            size="xs"
                                                                        />
                                                                        <NumberInput
                                                                            label="Height (ft)"
                                                                            value={win.height}
                                                                            onChange={(v) => updateWindow(win.id, { height: v })}
                                                                            step={0.5}
                                                                            min={0.5}
                                                                            size="xs"
                                                                        />
                                                                    ...
                                                                    <Select
                                                                        ...
                                                                        onChange={(v) => updateWindow(win.id, { material: v })}
                                                                        size="xs"
                                                                        mb="xs"
                                                                    />
                                                                    <Select
                                                                        ...
                                                                        data={SEALING}
                                                                        value={win.sealing}
                                                                        onChange={(v) => updateWindow(win.id, { sealing: v })}
                                                                        size="xs"
                                                                        mb="xs"
                                                                    />
                                                                    <Button size="xs" color="red" variant="light" onClick={() => deleteWindow(win.id)}>
                                                                        Remove
                                                                    </Button>
                                                                </Card>
                                                            ))}
                                                    ...
                                                    {wallDoors.length > 0 && (
                                                        <>
                                                            ...
                                                            {wallDoors.map((door, idx) => (
                                                                <Card key={door.id} withBorder padding="xs" bg="green.0">
                                                                    <Text size="xs" fw={500} mb="xs">Door {idx + 1}</Text>
                                                                    <Group grow mb="xs">
                                                                        <NumberInput
                                                                            label="Width (ft)"
                                                                            value={door.width}
                                                                            onChange={(v) => updateDoor(door.id, { width: v })}
                                                                            step={0.5}
                                                                            min={0.5}
                                                                            size="xs"
                                                                        />
                                                                        <NumberInput
                                                                            label="Height (ft)"
                                                                            value={door.height}
                                                                            onChange={(v) => updateDoor(door.id, { height: v })}
                                                                            step={0.5}
                                                                            min={0.5}
                                                                            size="xs"
                                                                        />
                                                                    ...
                                                                    <Select
                                                                        ...
                                                                        onChange={(v) => updateDoor(door.id, { material: v })}
                                                                        size="xs"
                                                                        mb="xs"
                                                                    />
                                                                    <Select
                                                                        ...
                                                                        data={SEALING}
                                                                        value={door.sealing}
                                                                        onChange={(v) => updateDoor(door.id, { sealing: v })}
                                                                        size="xs"
                                                                        mb="xs"
                                                                    />
                                                                    <Button size="xs" color="red" variant="light" onClick={() => deleteDoor(door.id)}>
                                                                        Remove
                                                                    </Button>
                                                                </Card>
                                                            ))}
                                                    ...
                                                    <Divider my="xs" />
                                                    <Button color="red" size="xs" variant="light" onClick={() => deleteWall(wall.id)}>
                                                        Delete Wall
                                                    </Button>
                                                </>
                                            );
                                    ...
                                    <Text c="dimmed" size="sm">Click a wall to edit, drag endpoints to position</Text>
                                )}
                            </Accordion.Panel>
                        </Accordion.Item>
                            ...
                            <Accordion.Panel>
                                <Stack gap="xs">
                                    <Alert color="blue" variant="light" mb="xs">
                                        Area auto-calculated: {floorAreaSqFt.toFixed(1)} sq ft ({floorAreaSqM.toFixed(1)} m²)
                                    </Alert>

                                    <Text fw={500} size="sm">Floor</Text>
                                    <Tooltip
                                    ...
                                    >
                                        <NumberInput
                                            ...
                                            onChange={(v) => updateFloorCeiling('floor', { u_value: v })}
                                            step={0.1}
                                            min={0.1}
                                            max={5}
                                            decimalScale={2}
                                    ...
                                    <Text fw={500} size="sm">Ceiling</Text>
                                    <Tooltip
                                    ...
                                    >
                                        <NumberInput
                                            ...
                                            onChange={(v) => updateFloorCeiling('ceiling', { u_value: v })}
                                            step={0.1}
                                            min={0.1}
                                            max={5}
                                            decimalScale={2}
                                ...
                                <Stack gap="xs">
                                    ...
                                    >
                                        <NumberInput
                                            ...
                                            onChange={(v) => updateVentilation({ air_changes_per_hour: v })}
                                            step={0.1}
                                            min={0}
                                            max={10}
                                            decimalScale={1}
